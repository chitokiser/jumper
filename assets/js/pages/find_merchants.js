...
async function init() {
    ...
    if (loaded) {
        ...
        if (mapEl) mapEl.innerHTML = "ÏßÄ???ïÎ≥¥Î•?Î∂àÎü¨?????ÜÏäµ?àÎã§.";
    }
    ...
    try {
        ...
        if (snap.empty) {
            grid.innerHTML = "<div style='color:#888; font-size:0.9rem; padding: 20px;'>?±Î°ù??Í∞ÄÎßπÏ†ê???ÜÏäµ?àÎã§.</div>";
            return;
        }
        ...
        snap.forEach(docSnap => {
            ...
            const websiteHtml = d.website ? `<a href="${d.website}" target="_blank" onclick="event.stopPropagation()" style="color:#2563eb; text-decoration:none;"><i class="fa-solid fa-globe me-1"></i> ?àÌéò?¥Ï? Î∞©Î¨∏</a>` : '';
            const emailHtml = d.email ? `<div style="font-size:12px; color:#64748b; margin-top:2px;">?âÔ∏è ${d.email}</div>` : '';
          ...
          <div class="mc-card-inner">
              ...
              <div class="mc-card-header-inner">
                  ...
                  <div>
                      <div class="mc-card-name">${d.name || "¿Ã∏ß æ¯¿Ω"}</div>
                      <div class="mc-card-career">${d.career || "πÃ∫–∑˘"} <span class="mc-card-region">${d.region || ""}</span></div>
                  </div>
              </div>
              
              <div class="mc-card-stats">
                 <span title="∞Ì∞¥ ∏Æ∫‰">?? ${reviews}</span>
                 <span title="¡¡æ∆ø‰">?? ${likes}</span>
                 <span title="KM ∞·¡¶ ºˆºˆ∑·" style="color:#10b981; font-weight:bold;">? ${kmFee}% ¿˚∏≥</span>
              </div>

              <div style="margin-top: 10px;">
                  <div class="mc-card-info-row">
                     <i class="fa-solid fa-phone" style="width:16px; text-align:center; color:#94a3b8;"></i> 
                     <span style="font-weight:600; color:#334155;">${d.phone || "π¯»£ πÃµÓ∑œ"}</span>
                  </div>
              ...
              <div class="mc-card-desc">${d.desc || "∞°∏Õ¡° º≥∏Ì¿Ã æ¯Ω¿¥œ¥Ÿ."}</div>
              
              ${d.website ? `
              <div class="mc-card-footer">
                 <a href="${d.website}" target="_blank" onclick="event.stopPropagation()" class="mc-link-btn">
                    <i class="fa-solid fa-globe"></i> »®∆‰¿Ã¡ˆ πÊπÆ
                 </a>
              </div>
              ` : ''}
          </div>
        ...
        $("merchantGrid").innerHTML = "<div style='color:#e53e3e; font-size:0.9rem; padding: 20px;'>Í∞ÄÎßπÏ†ê ?∞Ïù¥?∞Î? Î∂àÎü¨?§Îäî???§Ìå®?àÏäµ?àÎã§: " + err.message + "</div>";
    }
}
