...
import {
  initBattle, loadBattleData, loadDecorations, loadPlayerState,
...
import {
  initGameServer, connectToGameServer, disconnectFromGameServer,
...
import {
  initMoneyTree, refreshMoneyTreeInventory, loadMoneyTreeMarkers,
  renderMoneyTreeShopSection, openPlantModal
} from './merchants.moneytree.js';
import {
  initGoldMine, setGoldMineUid, loadGoldMineMarkers, openGoldMineModal,
  promptInstallMine, openMyGoldMinesModal
} from './merchants.goldmine.js?v=11';
import {
  initHarbor, setHarborUid, loadHarborMarkers, promptInstallHarbor,
  openHarborModal, openSvModal as openHarborSvModal
} from './merchants.harbor.js';
...
function renderMarkers(list) {
  ...
  list.forEach(m => {
    ...
    const marker = new google.maps.Marker({
      ...
      title: m.name || '',
      icon: {
        url: m.imageUrl || '/assets/images/jump/favicon.png',
...
function _makeBoxShadowIcon(size) {
  ...
  return {
    ...
    anchor: new google.maps.Point(sw / 2, 7),
  };
}
...
function openInventory() {
  ...
  // Firestore에서 최신 골드/포인트 동기화 (상점 판매 수익 반영)
...
// google.maps 로드 후에만 클래스 생성 (최상단 extends 평가 방지)
let _UserFaceMarkerOverlayClass = null;
function _getUserFaceMarkerOverlayClass() {
  if (_UserFaceMarkerOverlayClass) return _UserFaceMarkerOverlayClass;
  _UserFaceMarkerOverlayClass = class extends google.maps.OverlayView {
    constructor(data) {
      super();
      this._d = data;
      this._div = null;
  ...
  return _UserFaceMarkerOverlayClass;
}
...
async function loadUserMapMarkers() {
  ...
  try {
    ...
    _userMapMarkers = [];
    const OverlayClass = _getUserFaceMarkerOverlayClass();
    mkrs.forEach(m => {
      if (!m.lat || !m.lng) return;
      const overlay = new OverlayClass(m);
      overlay.setMap(map);
