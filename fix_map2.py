...
def fix_map():
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((pos) => {
      ...
      editMarker.setLatLng(userPos); // 과녁(저장포인트)을 내 위치로
      ...
      console.warn("GPS Location fetch disabled or failed", err);
    }, { enableHighAccuracy: true });
  }
}
"""

    # We match function initMap() {...} up to exactly `// ── 데이터 로드` or `async function loadBoxes()`
    # Let's search for the end bracket of initMap.
    match = re.search(r'function initMap\(\).*?editMarker\.on\(' + r"'dragend'.*?\}\);.*?\}", content, flags=re.DOTALL)
    if not match:
        print("Couldn't match.")
        return
        
    content = content[:match.start()] + new_init_map + content[match.end():]
