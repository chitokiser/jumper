...
const searchRegex = /const gsLevel = typeof bpSnap !== 'undefined'[\s\S]*?timestamp: admin\.firestore\.FieldValue\.serverTimestamp\(\) \}\);/m;
...
const r = `const gsLevel = typeof bpSnap !== 'undefined' && bpSnap.exists ? Math.max(1, Number(bpSnap.data().gsLevel || 1)) : 1;
    ...
    const isWinner = true; 
    const randomValue = grade; // Save grade in the random value slot for DB logging
    
    let newJackpotPool = currentJackpotPool + jackpotBonusVnd - jackpotRewardVnd;
    if (newJackpotPool < 0) {
        jackpotRewardVnd += newJackpotPool; // Cap reward so we don't go negative
        newJackpotPool = 0;
    }
...
if (searchRegex.test(js)) {
    js = js.replace(searchRegex, r);
    fs.writeFileSync('functions/handlers/transaction.js', js, 'utf8');
    console.log("Algorithm rewritten with regex");
} else {
